	...
	bool insert(unsigned key){
      ...
      buffer[n] = key;)
		...
		if(unlikely((heuristic[stateOff_]+=res)>=200))
		{
			...
			int lenlargeWhen;
			SYNC(vertype start; lb: start=llock_.startRead();) lenlargeWhen=enlargeWhen; SYNC(if(llock_.finishRead(start)==false) goto lb;)
...
//		__sync_synchronize();	
	...
	bool remove(unsigned key){
		...
		if(unlikely((heuristic[stateOff_]-=res)<-200))
      {
         int res = __sync_add_and_fetch(&fuzzySize, heuristic[stateOff_]);
			int lshrinkWhen;
         SYNC(vertype start; lb: start=llock_.startRead();) lshrinkWhen=shrinkWhen; SYNC(if(llock_.finishRead(start)==false) goto lb;)
