...
public:
	...
	long startRead() {
		repeat:
		long ret = std::atomic_load_explicit<long>(&version,std::memory_order_seq_cst);//read as volatile
		if(unlikely(ret & 1)){
			goto repeat;
		}
		//read memory fence.
	...
	bool finishRead(long start){
		long res = std::atomic_load_explicit<long>(&version,std::memory_order_seq_cst);
