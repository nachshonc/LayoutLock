...
public:
	...
	bool finishRead(std::atomic<char> *dirtyPtr){
		if(unlikely(std::atomic_load_explicit<char>(dirtyPtr,std::memory_order_seq_cst)!=0)){
			resetDirty();
			return false;
		}
		return true;
...
class SeqLock{
	ScalableRWLock<64> srwl;
	std::atomic<vertype> version;
	volatile int lock;
	static const int LOCKED_=1, UNLOCKED_=0;
public:
	static const bool ACTIVE=true;
	SeqLock() {
		version=0;
	...
	long startRead() {
		repeat:
		vertype ret = *(volatile vertype *)&version; 
		//vertype ret = std::atomic_load_explicit<vertype>(&version,std::memory_order_seq_cst);//read as volatile
		if(unlikely(ret & 1)){
			goto repeat;
		}
	...
	bool finishRead(vertype start){
		asm volatile("lfence":::"memory"); 
		vertype res = *(vertype *)&version; 
		//vertype res = std::atomic_load_explicit<vertype>(&version,std::memory_order_seq_cst);
		if(unlikely(start != res))
			return false;
		return true;
	}

	void startWrite() { assert(false); }

	void finishWrite() { assert(false);  }

	void startLayoutChange() {
		while (!__sync_bool_compare_and_swap(&lock, UNLOCKED_, LOCKED_)) ;
		version.fetch_add(1); 
	}

	void finishLayoutChange() {
		version.fetch_add(1);
		assert(__sync_bool_compare_and_swap(&lock, LOCKED_, UNLOCKED_));
	}
