...
#define DS_CONTAINS(k,r,t)  k->search(r, dirtyP) //bst_search(r, k)
#define DS_ADD(k,r,t)       k->insert(r, dirtyP) //bst_insert(r,(r+4),k)
#define DS_REMOVE(k,r,t)    k->remove(r, dirtyP) //bst_remove(r,k)
#define DS_SIZE(s)          s->size(s->head) //bst_size(s)
#define DS_NEW()            new layoutTree(0) //initialize_tree()
#define DS_LOCAL()          Layout_Lock::setup() //bst_init_local()

#define DS_TYPE             layoutTree
#define DS_NODE             noLockHelper::node
#define DS_KEY              unsigned
...
test(void* thread) 
{
  ...
  DS_TYPE* set = td->set;
  dptrtype *dirtyP = set->llock_.getDirtyP(); 
...
#define LLU long long unsigned int
  ...
  printf("#Mops %.3f. Duration=%ld\n", throughput / 1e6, duration);
